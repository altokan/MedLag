...
import { Camera, Image as ImageIcon, Send, X, CheckCircle, Upload } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto space-y-3 md:space-y-10 animate-in fade-in duration-500 pb-20 px-1 md:px-0">
      ...
      <form onSubmit={handleSubmit} className="bg-[#0d1b2e] p-4 md:p-12 rounded-2xl md:rounded-[56px] shadow-2xl border border-white/5 space-y-4 md:space-y-10">
        ...
        <div className="space-y-2">
           <label className="text-[8px] md:text-xs font-black text-slate-500 uppercase ml-1">Evidence</label>
           {photo ? (
             <div className="relative w-full aspect-video bg-[#0a1628] rounded-xl overflow-hidden group">
                <img src={photo} className="w-full h-full object-cover" />
                <button onClick={() => setPhoto(null)} className="absolute top-4 right-4 bg-red-600 text-white p-2 rounded-full shadow-lg"><X size={16}/></button>
             </div>
           ) : (
             <div className="grid grid-cols-2 gap-2 md:gap-6">
                <button type="button" onClick={() => cameraInputRef.current?.click()} className="cursor-pointer bg-[#0a1628] border border-dashed border-white/10 rounded-xl md:rounded-[32px] p-4 md:p-12 flex flex-col items-center justify-center space-y-1 md:space-y-4 text-slate-600 hover:text-[#ffd700] hover:border-[#ffd700]/30 transition-all">
                    <Camera size={18} className="md:w-12 md:h-12" />
                    <span className="text-[7px] md:text-xs font-black uppercase tracking-widest">Camera</span>
                </button>
                <button type="button" onClick={() => fileInputRef.current?.click()} className="cursor-pointer bg-[#0a1628] border border-dashed border-white/10 rounded-xl md:rounded-[32px] p-4 md:p-12 flex flex-col items-center justify-center space-y-1 md:space-y-4 text-slate-600 hover:text-[#ffd700] hover:border-[#ffd700]/30 transition-all">
                    <ImageIcon size={18} className="md:w-12 md:h-12" />
                    <span className="text-[7px] md:text-xs font-black uppercase tracking-widest">Upload</span>
                </button>
             </div>
           )}
